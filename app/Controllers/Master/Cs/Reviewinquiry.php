...
class Reviewinquiry extends MasterController
{
    protected $models = array('MrequestReviewinquiryModel');
    protected $viewPath = "mrequest/reviewinquiry";
    ...
    public function index()
    {
         ...
         * 검색 기능 구현 시작
         */
        if($this->sch_obj[1])$this->MrequestReviewinquiryModel->like("mrr_name",$this->sch_obj[1],'both'); // 이름
        if($this->sch_obj[2])$this->MrequestReviewinquiryModel->like("mes_medical_type",$this->sch_obj[2],'both'); // 연락처
        /* 검색 기능 구현 끝 */

        $data = $this->MrequestReviewinquiryModel->getPager();
    ...
    public function edit($idx=""){
        ...
        if(!$validate) {    //-- Form 출력
            // 데이터 불러오기
            if($idx){
                if(!$data = $this->MrequestReviewinquiryModel->find($idx))alert("데이터를 찾을 수 없습니다.");
            }
            else foreach($this->MrequestReviewinquiryModel->getAllowedFields() as $key=>$f)$data[$f]="";
            ...
            // 데이터 불러오기
            if($info[$this->primaryKey])$rs_info = $this->MrequestReviewinquiryModel->find($info[$this->primaryKey]);
